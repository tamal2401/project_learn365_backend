...
import org.learn365.exception.OtpVerifiedException;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler
{
    ...
    public ApplicationError otpVerificationException(
            OtpVerifiedException otpverifiedexception)
    {
        ApplicationError.ApplicationErrorBuilder error = new ApplicationError.ApplicationErrorBuilder(
                otpverifiedexception.getErrorcode(),
                otpverifiedexception.getMessage());
        return error.build();
    }

    @ExceptionHandler
    public ApplicationError otpException(
            OtpException otpexception)
    {
        ApplicationError.ApplicationErrorBuilder error = new ApplicationError.ApplicationErrorBuilder(
                otpexception.getErrorcode(),
    ...
    public ApplicationError handle(ConstraintViolationException exception)
    {
        ...
        String errorMessage = new ArrayList<>(
                exception.getConstraintViolations()).get(0).getMessage();
        ApplicationError.ApplicationErrorBuilder error = new ApplicationError.ApplicationErrorBuilder(
